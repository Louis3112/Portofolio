          ...
          <div className="px-2 pt-2 pb-6 space-y-2 sm:px-3 flex flex-col items-center">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className={`block px-3 py-2 font-mono text-xl ${activeSection === item.href.substring(1) ? 'text-neo-red font-bold' : 'text-neo-black'} hover:bg-neo-red hover:text-neo-white w-full text-center`}
